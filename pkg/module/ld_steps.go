...
func init() {
	m := monitor.NewPerfMonitor(&monitor.Monitor{
		...
		LazyInit: func(mm *monitor.Monitor, param ebpf.PreParam) bool {
			mm.Events = []*ebpf.Event{
				ebpf.NewUprobeEvent("bootstrap_finished", "__rtld_malloc_init_stubs", param.Interp, -1),
				ebpf.NewUretprobeEvent("start_user_prog", "_dl_start", param.Interp, -1),

				ebpf.NewUprobeEvent("dl_protect_relro", "_dl_protect_relro", param.Interp, -1),
				ebpf.NewUretprobeEvent("mprotect", "__mprotect", param.Interp, -1),
				ebpf.NewUretprobeEvent("mprotect", "mprotect", param.Interp, -1),
				ebpf.NewUretprobeEvent("ret_dl_protect_relro", "_dl_protect_relro", param.Interp, -1),

				// ebpf.NewUprobeEvent("munmap", "__munmap", param.Interp, -1),
				// ebpf.NewKprobeEvent("mmap", bpf.GetSyscallFnName("mmap"), -1),

				ebpf.NewUprobeEvent("dl_map_object_from_fd", "_dl_map_object_from_fd", param.Interp, -1),
				ebpf.NewUretprobeEvent("ret_dl_map_object_from_fd", "_dl_map_object_from_fd", param.Interp, -1),
			}
			mm.IsEnd = len(param.Interp) != 0 && strings.Contains(param.Interp, "ld-linux")
			return !mm.IsEnd
		},
	})
	m.RegisterOnceTable("bootstrap_finished_events", monitor.RenderHandler(bootstrapStepEvent{}, nil))
	m.RegisterOnceTable("start_user_prog_events", monitor.RenderHandler(startUserProgEvent{}, nil))
	m.RegisterTable("protect_relro_events", true, monitor.RenderHandler(protectRelroEvent{}, nil))

	m.RegisterTable("mprotect_events", true, monitor.RenderHandler(normalMprotectEvent{}, nil))

	m.RegisterTable("map_object_events", true, monitor.RenderHandler(mapObjectFromFdEvent{}, nil))
	// m.RegisterTable("mmap_events", true, monitor.RenderHandler(&MmapEvent{}))

	m.SetMark("start_user_prog_events", monitor.EndTag)
	factory.Register(m)
}
