...
func init() {
	m := monitor.NewPerfMonitor(&monitor.Monitor{
		...
		Events: []*ebpf.Event{
			ebpf.NewKprobeEvent("kprobe__setup_arg_pages", "setup_arg_pages", -1),
			ebpf.NewKretprobeEvent("kretprobe__arch_align_stack", "arch_align_stack", -1),
		},
	})
	m.RegisterOnceTable("setup_arg_pages_events", monitor.RenderHandler(setupArgPageEvent{}, nil))
	factory.Register(m)
}
